...
function _M.verify_response_memory(self, response_xml)
    ...
    local err = (function()
        ...
        for _, cert in ipairs(self.idp_certificates) do
        ...
        if self.id_attr ~= nil then
            local attrName = self.id_attr.attrName
            local nodeName = self.id_attr.nodeName
            local nsHref = self.id_attr.nsHref
